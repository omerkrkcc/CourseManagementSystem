...
{
    public partial class formDersOrtalama : Form
    {
        public formDersOrtalama()
        {
            InitializeComponent();
        }
        ...
        public void verileriListele(string txt,long ogrenciNumarasi)
        {
            NpgsqlCommand komut = new NpgsqlCommand();
            komut.Connection = sqlErisim;
            komut.CommandType = CommandType.Text;
            komut.Parameters.AddWithValue("@ogrenci_numarasi", ogrenciNumarasi);
        ...
        private void btnDersListele_Click(object sender, EventArgs e)
        {
            verileriListele("select ders_kodu,ogrenci_numarasi,ders_numarasi,ders_kredisi,donem," +
                   ...
                   "from ders_kayitlari where harfnotu is not null AND harfnotu!=' ' AND ogrenci_numarasi = @ogrenci_numarasi " +
                   "order by donem", Convert.ToInt64(txtOgrenciNo.Text));
